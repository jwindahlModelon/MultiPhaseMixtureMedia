...
function calcProperties_pFX "Return thermodynamic state record from p, F and X"
...
algorithm
 if (printExternalFunctionCall) then
   printMe:="calcProperties_pFX(p=" + String(p, significantDigits=8, minimumLength=0, leftJustified=true) +",F={";
  for i in 1:nP loop
    if
      (i == 1) then
