...
function calcProperties_duX "Return thermodynamic state record from d and u"
...
algorithm
 if (printExternalFunctionCall) then
   printMe:="calcProperties_duX(d=" + String(d, significantDigits=8, minimumLength=0, leftJustified=true) + ",u=" + String(u, significantDigits=8, minimumLength=0, leftJustified=true)+",X={";
  for i in 1:nC loop
    if
      (i == 1) then
