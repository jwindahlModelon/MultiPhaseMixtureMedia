...
function calcProperties_phX "Return thermodynamic state record from p and h"
...
algorithm
 if (printExternalFunctionCall) then
   printMe:="calcProperties_phX(p=" + String(p, significantDigits=8, minimumLength=0, leftJustified=true) + ",h=" + String(h, significantDigits=8, minimumLength=0, leftJustified=true)+",X={";
  for i in 1:nC loop
    if
      (i == 1) then
