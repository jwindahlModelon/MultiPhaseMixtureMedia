...
function calcProperties_phX_ext
  ...
  // Similar functionality sa before with calcProperties but difference is that overall properties is not calculated for most properties,
  ...
  // input Integer optimizationFlags[] - e.g only calculate overAllProps
  // phaseState
 //-----------overAllProperties--------------//
...
external"C" MultiPhaseMixtureMedium_calcThermoProperties_phX_C_impl(
      p,
      h,
      X,
      size(X, 1),
