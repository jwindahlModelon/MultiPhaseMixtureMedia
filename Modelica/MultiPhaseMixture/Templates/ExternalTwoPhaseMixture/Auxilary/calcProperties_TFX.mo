...
function calcProperties_TFX "Return thermodynamic state record from T, F and X"
  ...
  input Integer solutionType=1 "Solution type";
...
algorithm
 if (printExternalFunctionCall) then
   printMe:="calcProperties_TFX(p=" + String(T, significantDigits=8, minimumLength=0, leftJustified=true) +",F={";
  for i in 1:nP loop
    if
      (i == 1) then
      printMe:=printMe +String(F[i]);
    else
      printMe:=printMe + ","+String(F[i]);
    end if;
  end for;
