...
function calcProperties_pTX "Return thermodynamic state record from p and T"
...
algorithm
 if (printExternalFunctionCall) then
   printMe:="calcProperties_pTX(p=" + String(p, significantDigits=8, minimumLength=0, leftJustified=true) + ",T=" + String(T, significantDigits=8, minimumLength=0, leftJustified=true)+",X={";
  for i in 1:nC loop
    if
      (i == 1) then
