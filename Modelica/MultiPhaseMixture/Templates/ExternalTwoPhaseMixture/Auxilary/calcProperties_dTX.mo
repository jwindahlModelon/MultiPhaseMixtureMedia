...
function calcProperties_dTX "Return thermodynamic state record from p and T"
...
algorithm
 if (printExternalFunctionCall) then
   printMe:="calcProperties_dTX(d=" + String(d, significantDigits=8, minimumLength=0, leftJustified=true) + ",T=" + String(T, significantDigits=8, minimumLength=0, leftJustified=true)+",X={";
  for i in 1:nC loop
    if
      (i == 1) then
